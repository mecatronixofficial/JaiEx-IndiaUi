...
export default function LinksPage() {
  ...
  const counts = useMemo(() => ({
    all:      links.filter((l) => matchesShareMethod(l, methodFilter)).length,
    active:   links.filter((l) => matchesShareMethod(l, methodFilter) && l.status === "active").length,
    expired:  links.filter((l) => matchesShareMethod(l, methodFilter) && l.status === "expired").length,
    disabled: links.filter((l) => matchesShareMethod(l, methodFilter) && l.status === "disabled").length,
  }), [links, methodFilter]);
            ...
            {!loading && filtered.length > 0 && (
              <div className="border-t border-gray-100 px-5 py-3 dark:border-zinc-800">
                <p className="text-xs text-gray-500">
                  ...
                  {" "}of{" "}
                  <span className="font-semibold text-gray-700 dark:text-gray-200">{scopedLinks.length}</span>
