    ...
    <AuthGuard>
      <DashboardLayout>
        <div className="space-y-5 pb-10">
            ...
            {TYPE_OPTIONS.map((t) => {
              ...
              return (
                <button key={t.value} type="button" onClick={() => setType(t.value)}
                  className={cn(
                    "group flex flex-col gap-1.5 rounded-xl border p-3 text-left transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md",
                    active
                  ...
                  <span className={cn("transition-opacity", t.color, active ? "opacity-100" : "opacity-60 group-hover:opacity-100")}>
                    {t.icon}
                  </span>
                  <p className="text-[10px] font-medium text-(--text-muted)">{t.label}</p>
                  {loading
                    ? <div className="h-4 w-6 animate-pulse rounded bg-gray-100 dark:bg-zinc-800" />
          ...
          {/* ── Bulk action bar ── */}
          {selected.size > 0 && (
            <div className="flex flex-wrap items-center gap-3 rounded-xl border border-orange-200/80 bg-orange-50 px-4 py-3 dark:border-orange-900/30 dark:bg-orange-950/10">
              <div className="flex h-7 min-w-7 items-center justify-center rounded-lg bg-orange-500 px-2 text-xs font-bold text-white">
                {selected.size}
              </div>
              ...
              <button type="button" aria-label="Clear selection"
                ...
                className="flex h-6 w-6 items-center justify-center rounded-lg text-(--text-muted) hover:bg-(--bg-3) hover:text-(--text)">
                <X size={13} />
              </button>
