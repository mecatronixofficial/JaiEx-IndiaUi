...
function FileRow({ f, indent, uploading, onRemove }: FileRowProps) {
  return (
    ...
    >
      ...
      {f.status === "uploading" && (
        <span className="shrink-0 text-[11px] font-bold tabular-nums text-orange-500">{f.progress}%</span>
      )}
...
export default function UploadModal({
  ...
  async function uploadFile(
    ...
    try {
      ...
      const fileDoc = (res.data?.data ?? res.data) as Record<string, unknown> | undefined;
      const backendFileId: string | undefined =
        (fileDoc?.id as string | undefined) ??
        (fileDoc?._id as string | undefined)?.toString();
      const uploadSessionId: string | undefined =
        (fileDoc?.uploadSessionId as string | undefined) ??
        ((fileDoc?.file as Record<string, unknown> | undefined)?.uploadSessionId as string | undefined);
          ...
          <div className="flex flex-col items-center justify-center gap-4 px-6 py-14 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Drag &amp; drop, or browse · Any type · Up to 10 GB per file
              </p>
            ...
            {files.filter((f) => f.status === "uploading").map((f) => (
              <div key={`prog-${f.id}`} className="px-3 pb-1">
                <div className="h-1 overflow-hidden rounded-full bg-gray-200 dark:bg-zinc-700">
                  <div
                    className="bar-fill h-full rounded-full bg-orange-500 transition-all duration-200"
                    style={{ "--bar-w": `${f.progress}%` } as CSSProperties}
                  />
                </div>
              </div>
            ))}
